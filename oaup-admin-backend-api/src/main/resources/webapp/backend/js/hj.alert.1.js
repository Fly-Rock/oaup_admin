...
var ComAlertModal = {
    html:function(){
        var html =
            '<div class="modal fade" id="ComAlertModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">' +
            '<div class="modal-dialog">' +
            '<div class="modal-content">' +
            '<div class="modal-header hidden"><h4 class="modal-title"><strong>提示:</strong></h4></div>' +
            '<div class="modal-body"><div class="alert alert-success"></div></div>' +
    ...
    show:function(type, msg, callback){
        // dom init
        if ($('#ComAlertModal').length == 0){
            $('body').append(ComAlertModal.html());
        }

        // 弹框初始
        $('#ComAlertModal .alert').attr('class', 'alert alert-warning');
        $('#ComAlertModal .alert').html(msg);
        $('#ComAlertModal').modal('show');
